...
public class WorldGenMassiveTree extends WorldGenerator {
    ...
    public Block leaves = ModRegistry.Blocks.rubberLeaves;
    public Block log = ModRegistry.Blocks.rubberWood;
    ...
    public WorldGenMassiveTree(boolean doNotify) {
        super(doNotify);
    }
    ...
    private boolean validTreeLocation() {
        ...
        if (!block.canSustainPlant(state, world, pos, EnumFacing.UP, (BlockRubberSapling)ModRegistry.Blocks.rubberSapling))
            return false;
        else {
    ...
    public boolean generate(World world, Random par2Random, BlockPos pos) {
        ...
        if (!this.validTreeLocation())
            ...
            for (TLongObjectIterator<Chunk> iter = chunkMap.iterator(); iter.hasNext();) {
                iter.advance();
                IFGRetroMod.proxy.relightChunk(iter.value());
            }
            //time = System.nanoTime() - time;
